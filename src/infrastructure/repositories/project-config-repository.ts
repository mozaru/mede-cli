...
import { IProjectConfigRepository } from "../../domain/interfaces/repositories/project-config-repository-interface.js";
...
export class ProjectConfigRepository implements IProjectConfigRepository {
  ...
  public insert(projectConfigObj: ProjectConfigEntity): ProjectConfigEntity {
    ...
    const sql =
      "insert into projectConfig (projectId,medeConfigPath,hashContent,content,createdAt,updatedAt) values (@projectId,@medeConfigPath,@hashContent,@content,@createdAt,@updatedAt)";
  ...
  public updateContent(id: number, content: string, hashContent: string): boolean {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update projectConfig set content = @content,hashContent = @hashContent where id = @id";
    const result = this._uow.connection.prepare(sql).run({
      id: id,
      content: content,
  ...
  public list(projectId: number): ProjectConfigEntity[] {
    ...
    const sql =
      "select tp.id,tp.medeConfigPath,tp.hashContent,tp.content,tp.createdAt,tp.updatedAt,Project1.id as projectId from projectConfig tp left join Project Project1 on (tp.projectId = Project1.id)  where tp.projectId = @projectId";
    return this._uow.connection.prepare(sql).all({
      projectId: projectId,
    }) as ProjectConfigEntity[];
  }
  public getById(id: number): ProjectConfigEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.medeConfigPath,tp.hashContent,tp.content,tp.createdAt,tp.updatedAt,Project1.id as projectId from projectConfig tp left join Project Project1 on (tp.projectId = Project1.id)  where tp.id=@id";
  ...
  public getCurrent(projectId: number): ProjectConfigEntity | null {
    ...
    const row = this._uow.connection.prepare(sql).get({
      projectId: projectId,
    }) as ProjectConfigEntity | undefined;
    return row ?? null;
  }
