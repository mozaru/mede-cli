...
import { IProjectRepository } from "../../domain/interfaces/repositories/project-repository-interface.js";
...
export class ProjectRepository implements IProjectRepository {
  ...
  public insert(projectObj: ProjectEntity): ProjectEntity {
    ...
    const sql =
      "insert into project (name,rootProjectPath,docsRootPath,documentationLanguage,createdAt,updatedAt) values (@name,@rootProjectPath,@docsRootPath,@documentationLanguage,@createdAt,@updatedAt)";
  ...
  public update(projectObj: ProjectEntity): ProjectEntity {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update project set name=@name,rootProjectPath=@rootProjectPath,docsRootPath=@docsRootPath,documentationLanguage=@documentationLanguage,createdAt=@createdAt,updatedAt=@updatedAt where id=@id";
    this._uow.connection.prepare(sql).run({
      name: projectObj.name,
      rootProjectPath: projectObj.rootProjectPath,
      docsRootPath: projectObj.docsRootPath,
      documentationLanguage: projectObj.documentationLanguage,
  ...
  public list(): ProjectEntity[] {
    ...
    const sql =
      "select tp.id,tp.name,tp.rootProjectPath,tp.docsRootPath,tp.documentationLanguage,tp.createdAt,tp.updatedAt from project tp";
    return this._uow.connection.prepare(sql).all() as ProjectEntity[];
  }
  public getById(id: number): ProjectEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.name,tp.rootProjectPath,tp.docsRootPath,tp.documentationLanguage,tp.createdAt,tp.updatedAt from project tp where tp.id=@id";
  ...
  public getCurrent(): ProjectEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.name,tp.rootProjectPath,tp.docsRootPath,tp.documentationLanguage,tp.createdAt,tp.updatedAt from project tp order by tp.id desc limit 1";
