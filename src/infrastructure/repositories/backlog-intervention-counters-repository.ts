...
import { IBacklogInterventionCountersRepository } from "../../domain/interfaces/repositories/backlog-intervention-counters-repository-interface.js";
...
export class BacklogInterventionCountersRepository implements IBacklogInterventionCountersRepository {
  ...
  public insert(
    backlogInterventionCountersObj: BacklogInterventionCountersEntity,
  ): BacklogInterventionCountersEntity {
    this._uow.ensureTransactionForWrite();
    const sql =
      "insert into backlogInterventionCounters (projectId,name,lastNumber,createdAt,updatedAt) values (@projectId,@name,@lastNumber,@createdAt,@updatedAt)";
  ...
  public list(projectId: number): BacklogInterventionCountersEntity[] {
    ...
    const sql =
      "select tp.id,tp.name,tp.lastNumber,tp.createdAt,tp.updatedAt,Project4.id as projectId from backlogInterventionCounters tp left join Project Project4 on (tp.projectId = Project4.id)  where tp.projectId = @projectId";
    return this._uow.connection.prepare(sql).all({
      projectId: projectId,
    }) as BacklogInterventionCountersEntity[];
  }
  public getById(id: number): BacklogInterventionCountersEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.name,tp.lastNumber,tp.createdAt,tp.updatedAt,Project4.id as projectId from backlogInterventionCounters tp left join Project Project4 on (tp.projectId = Project4.id)  where tp.id=@id";
  ...
  public updateCounter(projectId: number, name: string, lastNumber: number): boolean {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update backlogInterventionCounters set lastNumber = @lastNumber,updatedAt = @updatedAt  where projectId = @projectId and name = @name";
    const result = this._uow.connection.prepare(sql).run({
      projectId: projectId,
      name: name,
