...
import { IChangeSetRepository } from "../../domain/interfaces/repositories/change-set-repository-interface.js";
...
export class ChangeSetRepository implements IChangeSetRepository {
  ...
  public insert(changeSetObj: ChangeSetEntity): ChangeSetEntity {
    ...
    const sql =
      "insert into changeSet (phaseId,cycleArtifactId,fileName,completed,currentChangeChunkIndex,changeChunkCount,currentOffset,startedAt,updatedAt) values (@phaseId,@cycleArtifactId,@fileName,@completed,@currentChangeChunkIndex,@changeChunkCount,@currentOffset,@startedAt,@updatedAt)";
  ...
  public updateComplete(id: number): boolean {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update changeSet set completed = @completed, updatedAt = @updatedAt  where id = @id";
    const result = this._uow.connection.prepare(sql).run({
      id: id,
      completed: 1,
  ...
  public updateChunkIndex(
    id: number,
    currentChangeChunkIndex: number,
    currentOffset: number,
  ): boolean {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update changeSet set currentChangeChunkIndex = @currentChangeChunkIndex, currentOffset = @currentOffset, updatedAt = @updatedAt  where id = @id";
    const result = this._uow.connection.prepare(sql).run({
      id: id,
