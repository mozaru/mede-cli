...
import { ILlmProfileRepository } from "../../domain/interfaces/repositories/llm-profile-repository-interface.js";
...
export class LlmProfileRepository implements ILlmProfileRepository {
  ...
  public insert(llmProfileObj: LlmProfileEntity): LlmProfileEntity {
    ...
    const sql =
      "insert into llmProfile (projectId,profileName,provider,model,endpoint,apiKeyEnv,temperature,maxTokens,timeoutMs,retryJson,active) values (@projectId,@profileName,@provider,@model,@endpoint,@apiKeyEnv,@temperature,@maxTokens,@timeoutMs,@retryJson,@active)";
  ...
  public list(projectId: number): LlmProfileEntity[] {
    ...
    const sql =
      "select tp.id,tp.profileName,tp.provider,tp.model,tp.endpoint,tp.apiKeyEnv,tp.temperature,tp.maxTokens,tp.timeoutMs,tp.retryJson,tp.active,Project2.id as projectId from llmProfile tp left join Project Project2 on (tp.projectId = Project2.id)  where tp.projectId = @projectId";
  ...
  public getById(id: number): LlmProfileEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.profileName,tp.provider,tp.model,tp.endpoint,tp.apiKeyEnv,tp.temperature,tp.maxTokens,tp.timeoutMs,tp.retryJson,tp.active,Project2.id as projectId from llmProfile tp left join Project Project2 on (tp.projectId = Project2.id)  where tp.id=@id";
