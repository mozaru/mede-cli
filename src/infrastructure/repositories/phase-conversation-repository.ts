...
import { IPhaseConversationRepository } from "../../domain/interfaces/repositories/phase-conversation-repository-interface.js";
...
export class PhaseConversationRepository implements IPhaseConversationRepository {
  ...
  public insert(phaseConversationObj: PhaseConversationEntity): PhaseConversationEntity {
    ...
    const sql =
      "insert into phaseConversation (phaseId,createdAt,actor,content) values (@phaseId,@createdAt,@actor,@content)";
  ...
  public list(phaseId: number): PhaseConversationEntity[] {
    ...
    const sql =
      "select tp.id,tp.createdAt,tp.actor,tp.content,Phase8.id as phaseId from phaseConversation tp left join Phase Phase8 on (tp.phaseId = Phase8.id)  where tp.phaseId = @phaseId";
    return this._uow.connection.prepare(sql).all({
      phaseId: phaseId,
    }) as PhaseConversationEntity[];
  }
  public getById(id: number): PhaseConversationEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.createdAt,tp.actor,tp.content,Phase8.id as phaseId from phaseConversation tp left join Phase Phase8 on (tp.phaseId = Phase8.id)  where tp.id=@id";
    const row = this._uow.connection.prepare(sql).get({ id: id }) as
      | PhaseConversationEntity
      | undefined;
    return row ?? null;
  }
