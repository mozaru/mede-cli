...
import { IPhaseAttachmentRepository } from "../../domain/interfaces/repositories/phase-attachment-repository-interface.js";
...
export class PhaseAttachmentRepository implements IPhaseAttachmentRepository {
  ...
  public insert(phaseAttachmentObj: PhaseAttachmentEntity): PhaseAttachmentEntity {
    ...
    const sql =
      "insert into phaseAttachment (phaseId,createdAt,actor,filePath,fileName,content,contentText) values (@phaseId,@createdAt,@actor,@filePath,@fileName,@content,@contentText)";
  ...
  public list(phaseId: number): PhaseAttachmentEntity[] {
    ...
    const sql =
      "select tp.id,tp.createdAt,tp.actor,tp.filePath,tp.fileName,tp.content,tp.contentText,Phase9.id as phaseId from phaseAttachment tp left join Phase Phase9 on (tp.phaseId = Phase9.id)  where tp.phaseId = @phaseId";
    return this._uow.connection.prepare(sql).all({
      phaseId: phaseId,
    }) as PhaseAttachmentEntity[];
  }
  public getById(id: number): PhaseAttachmentEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.createdAt,tp.actor,tp.filePath,tp.fileName,tp.content,tp.contentText,Phase9.id as phaseId from phaseAttachment tp left join Phase Phase9 on (tp.phaseId = Phase9.id)  where tp.id=@id";
    const row = this._uow.connection.prepare(sql).get({ id: id }) as
      | PhaseAttachmentEntity
      | undefined;
    return row ?? null;
  }
