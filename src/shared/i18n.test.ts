...
describe("I18n Localization & Translation Sweeper", () => {
  ...
  it("should ensure static dictionaries (pt-BR/messages.json and en-US/messages.json) are fully aligned in translation keys", () => {
    const ptContent = fs.readFileSync(path.join(PACKAGE_LOCALES_DIR, "pt-BR", "messages.json"), "utf-8");
    const enContent = fs.readFileSync(path.join(PACKAGE_LOCALES_DIR, "en-US", "messages.json"), "utf-8");
