...
export class OpenRouterPkceFlow {
  ...
  public async login(): Promise<string> {
    ...
    const port = this.config.callbackPort ?? DEFAULT_PORT;
    ...
    if (!response.ok || !body.key) {
      throw new Error(
        `Falha ao trocar o código OpenRouter por uma key: ${body.error ?? `HTTP ${response.status}`}.`,
      );
    }
  ...
  return (authUrl) =>
    new Promise<string>((resolve, reject) => {
      const server = http.createServer((req, res) => {
        ...
        res.end(
          code
            ? "<html><body>Login concluído. Você já pode fechar esta aba.</body></html>"
            : "<html><body>Login sem código. Você pode fechar esta aba.</body></html>",
        );
