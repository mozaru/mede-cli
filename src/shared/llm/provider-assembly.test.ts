...
describe('AnthropicLlmProvider message assembly', () => {
  ...
  it('system prompt is separate from context messages', () => {
    const { system, anthropicMessages } = (provider as any).buildRequestMessages()
    expect(system).toBe('SYSTEM PROMPT')
    // system must not appear in the messages array
    expect(anthropicMessages.every((m: { content: string }) => m.content !== 'SYSTEM PROMPT')).toBe(true)
  })

  it('builds messages in correct order: context → extraInfo → userPrompt', () => {
    const { anthropicMessages } = (provider as any).buildRequestMessages()
    const contents: string[] = anthropicMessages.map((m: { content: string }) => m.content)
...
describe('OpenAiLlmProvider message assembly', () => {
  ...
  it('builds messages in correct order: system → context → extraInfo → userPrompt', () => {
    const messages: Array<{ role: string; content: string }> = (provider as any).buildRequestMessages()
...
describe('GeminiLlmProvider message assembly', () => {
  ...
  it('system prompt is in systemInstruction (separate from contents)', () => {
    const { systemInstruction, contents } = (provider as any).buildRequestMessages()
    expect(systemInstruction.parts[0].text).toBe('SYSTEM PROMPT')
    expect(contents.every((c: { parts: Array<{ text: string }> }) =>
      c.parts[0].text !== 'SYSTEM PROMPT'
    )).toBe(true)
  })

  it('builds contents in correct order: context → extraInfo → userPrompt', () => {
    const { contents } = (provider as any).buildRequestMessages()
    const texts: string[] = contents.map((c: { parts: Array<{ text: string }> }) => c.parts[0].text)
