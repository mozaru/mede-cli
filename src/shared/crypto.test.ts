...
describe("calculateHashFromJson", () => {
  ...
  it("hashes the JSON serialization of an object", () => {
    const value = { a: 1, b: 2 };
    expect(calculateHashFromJson(value)).toBe(calculateHashFromContent(JSON.stringify(value)));
  });

  it("is sensitive to property order (string serialization)", () => {
    expect(calculateHashFromJson({ a: 1, b: 2 })).not.toBe(calculateHashFromJson({ b: 2, a: 1 }));
  });
});
