...
describe("StatusService unit tests", () => {
  it("formats and generates project status report correctly based on project, cycle and phase state", () => {
    // 1. Mocks
    ...
    expect(output).toContain("Language     : pt-BR");
    expect(output).toContain("State        : aguardando approve/reject");
    expect(output).toContain("Proposal     : não vazia");
    expect(output).toContain("Refinements  : 2");
    expect(output).toContain("Changed files: 1");
    expect(output).toContain("Created files: 1");
