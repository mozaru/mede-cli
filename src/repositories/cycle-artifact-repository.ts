...
export class CycleArtifactRepository implements ICycleArtifactRepository {
  ...
  public insert(cycleArtifactObj: CycleArtifactEntity): CycleArtifactEntity {
    ...
    const sql =
      "insert into cycleArtifact (cycleId,backupContent,currentContent,canonicalName,canonicalType,artifactPath,startedAt,updatedAt) values (@cycleId,@backupContent,@currentContent,@canonicalName,@canonicalType,@artifactPath,@startedAt,@updatedAt)";
  ...
  public list(cycleId: number): CycleArtifactEntity[] {
    ...
    const sql =
      "select tp.id,tp.backupContent,tp.currentContent,tp.canonicalName,tp.canonicalType,tp.artifactPath,tp.startedAt,tp.updatedAt,Cycle7.id as cycleId from cycleArtifact tp left join Cycle Cycle7 on (tp.cycleId = Cycle7.id)  where tp.cycleId = @cycleId";
    return this._uow.connection.prepare(sql).all({
      cycleId: cycleId,
    }) as CycleArtifactEntity[];
  }
  public getById(id: number): CycleArtifactEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.backupContent,tp.currentContent,tp.canonicalName,tp.canonicalType,tp.artifactPath,tp.startedAt,tp.updatedAt,Cycle7.id as cycleId from cycleArtifact tp left join Cycle Cycle7 on (tp.cycleId = Cycle7.id)  where tp.id=@id";
    const row = this._uow.connection.prepare(sql).get({ id: id }) as
      | CycleArtifactEntity
      | undefined;
    return row ?? null;
  }
  public getFromPath(cycleId: number, artifactPath: string): CycleArtifactEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.backupContent,tp.currentContent,tp.canonicalName,tp.canonicalType,tp.artifactPath,tp.startedAt,tp.updatedAt,Cycle7.id as cycleId from cycleArtifact tp left join Cycle Cycle7 on (tp.cycleId = Cycle7.id)  where tp.cycleId = @cycleId and tp.artifactPath = @artifactPath";
    const row = this._uow.connection.prepare(sql).get({
      cycleId: cycleId,
      artifactPath: artifactPath,
    }) as CycleArtifactEntity | undefined;
    return row ?? null;
  }
  ...
  public updateContent(id: number, currentContent: string): boolean {
    this._uow.ensureTransactionForWrite();
    const sql =
      "update cycleArtifact set currentContent = @currentContent,updatedAt = @updatedAt  where id = @id";
    const result = this._uow.connection.prepare(sql).run({
      id: id,
  ...
  public existAnyByCycle(cycleId: number): boolean {
    ...
    return (
      (this._uow.connection.prepare(sql).get({
        cycleId: cycleId,
      }) as number) > 0
    );
  }
