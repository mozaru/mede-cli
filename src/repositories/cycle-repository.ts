...
export class CycleRepository implements ICycleRepository {
  ...
  public insert(cycleObj: CycleEntity): CycleEntity {
    ...
    const sql =
      "insert into cycle (projectId,status,currentPhaseIndex,phaseCount,autoMode,startedAt,finishedAt) values (@projectId,@status,@currentPhaseIndex,@phaseCount,@autoMode,@startedAt,@finishedAt)";
  ...
  public list(projectId: number): CycleEntity[] {
    ...
    const sql =
      "select tp.id,tp.status,tp.currentPhaseIndex,tp.phaseCount,tp.autoMode,tp.startedAt,tp.finishedAt,Project5.id as projectId from cycle tp left join Project Project5 on (tp.projectId = Project5.id)  where tp.projectId = @projectId";
  ...
  public getById(id: number): CycleEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.status,tp.currentPhaseIndex,tp.phaseCount,tp.autoMode,tp.startedAt,tp.finishedAt,Project5.id as projectId from cycle tp left join Project Project5 on (tp.projectId = Project5.id)  where tp.id=@id";
  ...
  public getCurrent(projectId: number): CycleEntity | null {
    this._uow.ensureConnection();
    const sql =
      "select tp.id,tp.status,tp.currentPhaseIndex,tp.phaseCount,tp.autoMode,tp.startedAt,tp.finishedAt,Project5.id as projectId from cycle tp left join Project Project5 on (tp.projectId = Project5.id)  where tp.projectId = @projectId and tp.status IN ('OPEN', 'AWAITING_COMMIT')";
    const row = this._uow.connection.prepare(sql).get({
      projectId: projectId,
    }) as CycleEntity | undefined;
    return row ?? null;
  }
